...
export const CSVDownloadButton_1: React.FC<{ data: DataObject[] }> = ({ data }) => {
    const [fileName, setFileName] = useState('_quantity_sold.csv');

    const handleDownload = () => {
        const csvData = convertArrayOfObjectsToCSV(data);
        downloadCSV(csvData, _getTime()+fileName);
    };
...
const generateCSV = (data: any[][], fileName: string) => {
  ...
  const CSV_2_TableComponent: React.FC<Props> = ({ data }) => {
    const [fileName, setFileName] = useState('_analysis_data.csv');
  ...
  export default CSV_2_TableComponent;